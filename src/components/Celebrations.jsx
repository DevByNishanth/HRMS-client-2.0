...
const Celebrations = () => {
  ...
  const fetchCelebrations = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.message || "Failed to load celebrations.");
      }
        ...
        {loading ? (
          ...
          >
            {filteredCelebrations.map((celebration, index) => {
              const Icon = celebration.type === "Birthday" ? Cake : Briefcase;

              const name = [celebration.firstName, celebration.lastName]
                .filter(Boolean)
                .join(" ");

              const { month, day } = formatCelebrationDate(celebration.date);

              const departmentLabel = celebration.department
