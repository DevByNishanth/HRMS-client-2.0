...
const FilterDatePicker = ({
  ...
  const moveMonth = (dir) =>
    setViewDate((cd) => new Date(cd.getFullYear(), cd.getMonth() + dir, 1));
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-3 table-custom-scrollbar">
          ...
          <div className="mt-2 rounded-lg border border-[#1d395e] bg-[#0a1a2d] p-3 shadow-[0_12px_26px_rgba(0,0,0,0.16)]">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-semibold uppercase ${compOff.statusColor}`}
              >
                ...
                {compOff.approvalHistory.map((history, index) => {
                  ...
                  return (
                    <div key={index} className="relative">
                      {!isLast && (
                        <div
                          className={`absolute left-[19px] top-[50px] w-[2px] h-[60px] ${bgColor}`}
                        />
                      )}
...
const mapApiRequest = (req) => {
  ...
  const rejectionEntry = (req.approvalHistory || []).find(
    (h) => h.action?.toLowerCase() === "rejected",
  );
...
const FacultyCompOffTable = () => {
  ...
  const fetchCompOffs = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL.replace(/\/$/, "")}/api/comp-off/me`,
        {
  ...
  const filteredData = useMemo(
    () =>
      data.filter((item) => {
        ...
        const ifd = n(item.fromDate),
          itd = n(item.toDate);
        const ffn = filterFromDate
          ? new Date(
              Date.UTC(
  ...
  const handleRevoke = async () => {
    ...
    try {
      const token = getTokenFromLocalStorage();
      const response = await fetch(
        `${API_BASE_URL.replace(/\/$/, "")}/api/comp-off/${revokeTarget._id}/withdraw`,
        {
          method: "PATCH",
          headers: {
  ...
  return (
    <>
      <section className="rounded-xl border border-[#183052] max-h-[calc(100vh-210px)] min-h-[calc(100vh-210px)] bg-[#0a1a2d] mt-3 ">
        ...
        <div className="relative z-0 max-h-[calc(100vh-280px)] overflow-auto table-custom-scrollbar">
          {loading ? (
            ...
            <div className="flex items-center justify-center py-12 text-[#f16868]">
              {error}
            </div>
          ) : (
              ...
              <tbody className="text-[12px] text-[#cad7eb]">
                {filteredData.length > 0 ? (
                  filteredData.map((item, i) => {
                    const iwc = {
                      ...item,
                      statusColor: statusStyles[item.status],
                    };
                    return (
                      <tr
                        key={i}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            ...
                            className={`inline-flex cursor-pointer items-center gap-1 text-[13px] font-medium underline transition ${item.documentUrl ? "text-[#3984ff] hover:text-[#6ea1ff]" : "text-[#6f839f] cursor-default"}`}
                          >
                            {item.documentUrl ? "View Doc" : "—"}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[13px] font-semibold ${statusStyles[item.status]}`}
                          >
      ...
      <CompOffDetailsCanvas
        compOff={selectedCompOff}
        onClose={() => setSelectedCompOff(null)}
      />
      {revokeTarget && (
        <section
              ...
              <div className="flex gap-3 rounded-lg border border-[#f0a15f40] bg-[#f0a15f12] p-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[12px] leading-5 text-[#b8c7dd]">
                    ...
                    to{" "}
                    {revokeTarget.toDate
                      ? new Date(revokeTarget.toDate).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "2-digit", year: "numeric" },
                        )
                      : ""}
