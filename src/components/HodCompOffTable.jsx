...
const ConfirmationPopup = ({
  ...
  const title = isReject ? "Reject Comp-Off Request" : "Revoke Comp-Off Decision";
      ...
      >
        ...
        <div className="flex justify-end gap-3 border-t border-[#173150] px-5 py-4">
          ...
          <button
            type="button"
            onClick={onConfirm}
            disabled={(isReject && !reason.trim()) || confirmLoading || revokeLoading}
...
const HodCompOffTable = ({ onCountChange }) => {
  ...
  const getDepartmentFromToken = () => {
    try {
      ...
      return decoded?.department || decoded?.dept || "CSE";
    } catch {
      return "CSE";
    }
  };
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter((request) => {
      ...
      return statusMatch && filterDateCheck && searchMatch;
    });
  }, [filterStatus, filterDate, searchQuery, requests]);
  ...
  const handleConfirmAction = async () => {
    ...
    if (action === "reject") {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL.replace(/\/$/, "")}/api/comp-off/${request.id}/reject`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL.replace(/\/$/, "")}/api/comp-off/${request.id}/revoke-hod`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!response.ok) {
          throw new Error("Failed to revoke comp-off decision");
        }
        toast.success("Comp-off decision revoked successfully");
        closeConfirmation();
        await fetchCompOffs();
      } catch (err) {
        toast.error(err.message || "Failed to revoke decision");
      } finally {
        setRevokeLoading(false);
      }
    }
  ...
  return (
    <>
      <section className="rounded-xl border border-[#244061]  bg-[#0a1a2d] mt-4">
