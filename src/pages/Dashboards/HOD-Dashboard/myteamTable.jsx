...
const presenceStyles = {
  ...
  "Not Marked": "text-[#9aacc7] bg-[#9aacc71f]",
};
...
const MyTeamTable = ({ teamMembers = [], loading = false }) => {
  ...
  // Get unique options from data (dynamic based on API response)
  const presenceOptions = ["All", ...new Set(teamMembers.map((m) => m.presence))];
  const roleOptions = ["All", ...new Set(teamMembers.map((m) => m.designation))];
  ...
  const filteredMembers = useMemo(() => {
    return teamMembers.filter((member) => {
      ...
      const roleMatch = filterRole === "All" || member.designation === filterRole;
  ...
  }, [searchQuery, filterPresence, filterRole, filterType, teamMembers]);
                ...
                filteredMembers.map((member) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-semibold ${presenceStyles[member.presence] || "text-[#9aacc7] bg-[#9aacc71f]"}`}
                      >
