                                ...
                                {request.approvalHistory.map((history, index) => {
                                    ...
                                    return (
                                        <div key={index} className="relative">
                                            ...
                                            {!isLast && (
                                                <div
                                                    className={`absolute left-[19px] top-10 bottom-0 w-[2px] ${isApproved ? "bg-[#10b981]" : isRejected ? "bg-[#ef4444]" : "bg-slate-200 dark:bg-[#444c63]"
                                                        }`}
                                                />
                                            )}
...
const HodLeaveRequestTable = ({ onCountChange }) => {
    ...
    const exportCurrentFilteredRows = () => {
        const rows = filteredRequests.map((req) => ({
            ...
            "Reason": req.reason || "",
            "Status": req.approvalStatus?.hodStatus || req.status || "Pending",
        }));
        exportToExcel(rows, "Team-Leave-Requests.xlsx");
    };
