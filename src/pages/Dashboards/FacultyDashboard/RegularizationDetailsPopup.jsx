        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 table-custom-scrollbar">
          <div className="rounded-lg border border-slate-200 dark:border-[#1d395e] bg-slate-50 dark:bg-[#0a1a2d] p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[12px] font-medium text-slate-500 dark:text-[#8ca1bd]">{request.facultyId?.empId}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 rounded-lg border border-slate-200 dark:border-[#1d395e] bg-slate-50 dark:bg-[#0a1a2d] p-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="flex items-center gap-2 text-[12px] font-medium text-slate-500 dark:text-[#9eb0cc]">Date</div>
                <p className="mt-1 text-[15px] font-medium text-gray-900 dark:text-white">{formatDate(request.attendanceDate)}</p>
              </div>
              <div>
                <div className="flex items-center gap-2 text-[12px] font-medium text-slate-500 dark:text-[#9eb0cc]">Status</div>
                <p className="mt-1 text-[15px] font-medium text-gray-900 dark:text-white">{request.status}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 rounded-lg border border-slate-200 dark:border-[#1d395e] bg-slate-50 dark:bg-[#0a1a2d] p-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="flex items-center gap-2 text-[12px] font-medium text-slate-500 dark:text-[#9eb0cc]">In Time</div>
                <p className="mt-1 text-[15px] font-medium text-[#18d3bf]">{formatTime(request.requestedInTime)}</p>
              </div>
              <div>
                <div className="flex items-center gap-2 text-[12px] font-medium text-slate-500 dark:text-[#9eb0cc]">Out Time</div>
                ...
                {request.approvalHistory.map((history, index) => {
                  ...
                  return (
                    <div key={index} className="relative">
                      {!isLast && <div className={`absolute left-[19px] top-10 bottom-0 w-[2px] ${isApproved ? "bg-[#10b981]" : isRejected ? "bg-[#ef4444]" : "bg-slate-300 dark:bg-slate-200 dark:bg-[#444c63]"}`} />}
                      <div className="relative flex gap-3 pb-6">
                        <div className="flex-shrink-0">
                          <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 ${isApproved ? `${actionColor.bg} border-emerald-200/20` : isRejected ? `${actionColor.bg} border-[#ef4444]` : `${actionColor.light} border-slate-300 dark:border-[#444c63]`} text-white`}>
