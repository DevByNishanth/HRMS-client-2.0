...
const DropdownField = ({
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      >
        <span className={getDisplayValue() ? "text-[var(--theme-text-main)]" : "text-[var(--theme-text-muted)]"}>
      ...
      {isOpen && (
        <>
          ...
          <div
            className={`absolute left-0 right-0 z-40 max-h-[220px]  overflow-y-auto rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-card)] py-1 shadow-[0_18px_45px_rgba(0,0,0,0.35)] table-custom-scrollbar ${positionAbove ? "bottom-[calc(100%+8px)]" : "top-[calc(100%+8px)]"
              }`}
          >
            {options.map((option) => {
          ...
          {expandedOptionData?.submenu?.length > 0 && expandedOption && submenuPosition && (
            <div
              ...
              className="z-[60] rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-body)] shadow-[0_18px_45px_rgba(0,0,0,0.35)]"
            >
...
const ObjectDropdownField = ({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={selectedOption ? "text-[var(--theme-text-main)]" : "text-[var(--theme-text-muted)]"}>
          {isLoading
            ? "Loading..."
            : selectedOption
      ...
      {isOpen && (
        <>
          <button
            ...
            aria-label="Close dropdown"
          />
          <div className={`absolute left-0 right-0 z-40 max-h-[220px] overflow-y-auto rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-card)] py-1 shadow-[0_18px_45px_rgba(0,0,0,0.35)] table-custom-scrollbar ${positionAbove ? "bottom-[calc(100%+8px)]" : "top-[calc(100%+8px)]"
            }`}>
            {options.length > 0 ? (
              options.map((option) => {
              ...
              <p className="px-4 py-3 text-[13px] text-[var(--theme-text-muted)]">
  ...
  return (
    <div className="relative col-span-2 mb-2">
      ...
      <button
        ...
        type="button"
        onClick={() => updateOpenState(!isOpen)}
        className="flex h-11 w-full items-center justify-between rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-input)] px-3 text-left text-[13px] text-[var(--theme-text-main)] outline-none transition hover:border-[#3984ff] focus:border-[#3984ff] focus:ring-2 focus:ring-[#3984ff33]"
      >
      ...
      {isOpen && (
        <>
          ...
          />
          <div
            className={`absolute left-0 right-0 z-40 overflow-hidden rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-card)] shadow-[0_18px_45px_rgba(0,0,0,0.35)] ${positionAbove
              ? "bottom-[calc(100%+8px)]"
              : "top-[calc(100%+8px)]"
              }`}
          >
            <div className="relative border-b border-[var(--theme-border)] p-2">
              <Search
                size={15}
                className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--theme-text-muted)]"
              />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search employee..."
                className="h-10 w-full rounded-lg border border-[var(--theme-border-input)] bg-[var(--theme-bg-input)] pl-9 pr-3 text-[13px] text-[var(--theme-text-main)] outline-none placeholder:text-[var(--theme-text-muted)] focus:border-[#3984ff]"
              />
            </div>
...
const DateField = ({ id, label, required, value, onChange, placeholder, popupAlign, error }) => (
  <div>
    <div className="mb-2 block text-[13px] font-semibold text-[var(--theme-text-main)]">
      {label} {required && <span className="text-[#3984ff]">*</span>}
    </div>
...
const SectionTitle = ({ title, description }) => (
  <div className="mb-4">
    <h3 className="text-[15px] font-semibold text-[var(--theme-text-main)]">{title}</h3>
    <p className="mt-1 text-[12px] leading-5 text-[var(--theme-text-muted)]">{description}</p>
  </div>
);
...
const AddFacultyForm = ({
  ...
  return (
    ...
    >
      <form
        className="flex h-full w-[60%] xl:w-[42%] flex-col bg-[var(--theme-bg-body)] shadow-[-18px_0_50px_rgba(0,0,0,0.35)]"
                  ...
                  {qualifications.map((qualification, index) => (
                    ...
                    >
                      <div className="mb-4 flex items-center justify-between">
                        <h4 className="text-[13px] font-semibold text-[var(--theme-text-main)]">
