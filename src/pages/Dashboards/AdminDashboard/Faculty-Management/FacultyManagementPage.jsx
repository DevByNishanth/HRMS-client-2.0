...
const FacultyManagementPage = () => {
    ...
    const typeOptions = useMemo(
        () => [
            "All",
            ...Array.from(
                new Set(facultyMembers.map((faculty) => faculty.employeeCategory).filter(Boolean)),
            ),
        ],
        [facultyMembers],
    );
    ...
    const filteredFaculty = useMemo(() => {
        ...
        return facultyMembers.filter((faculty) => {
            ...
            const matchesSearch =
                !normalizedSearch ||
                [name, faculty.empId, faculty.designation, faculty.department, faculty.employeeCategory]
                    .join(" ")
                    .toLowerCase()
    ...
    }, [departmentFilter, facultyMembers, roleFilter, searchQuery, typeFilter]);
    ...
    const handleDeleteFaculty = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data?.message || data?.error || "Unable to delete faculty.");
            }

            setDeletingFaculty(null);
            fetchFaculties();
        } catch (error) {
            setDeleteError(error.message || "Something went wrong while deleting faculty.");
        } finally {
